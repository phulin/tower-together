...
1. `compute_tower_tier_from_ledger() > g_star_count` — ledger total exceeds the next tier threshold
...
via `place_cathedral_stack` at `1200:2347` creates 5 placed-object records on consecutive
floors with types 0x24 (bottom) through 0x28 (top), each 28 tiles wide. These are
per-floor slices of a single building, not separate facilities.
...
0x000): when `g_eval_entity_index >= 0` and `star_count > 2`, sweeps floors 0x6d–0x77
(109–119) for types 0x24–0x28 and forces all 40 entity slots into state 0x20.
...
guaranteed after tick 240. Entities still in state 0x20 at `daypart_index >= 1` are
parked (state 0x27) and must wait for the next day.

**Outbound journey**: each entity routes from floor 10 (lobby) to floor 0x6d (109).
...
- Iterates all floors 0x6d–0x77, counts entities in state 0x03 for types 0x24–0x28
...
0x04b0): all entities in state 0x03 are advanced to state 0x05 and route back to floor 10.
...
- `compute_tower_tier_from_ledger() > g_star_count` (ledger total >= 15000 for tier 6)

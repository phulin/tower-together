...
- **GameScreen.tsx** — Main game screen composition root. Owns local view state such as selected tool, alias-edit form state, and toast queue, while delegating tower session/socket state to `useTowerSession.ts` and rendering smaller presentational subcomponents for the toolbar, debug HUD, prompts, inspection dialog, toasts, and status bar.
- **useTowerSession.ts** — React hook for the active tower session. Subscribes to socket messages/status, updates scene snapshots/presentation clock, stores session-facing UI state (cash, sim time, prompts, inspected cell, entities/carriers), and exposes command helpers back to `GameScreen`.
